<x-layout>
    @section('title', $post->getTrans('title'))
    @section('meta_description', $post->getTrans('meta_description') ?? Str::limit(strip_tags($post->getTrans('content')), 160))
    @if($post->featured_image)
        @section('meta_image', Str::startsWith($post->featured_image, 'http') ? $post->featured_image : asset('storage/' . $post->featured_image))
    @endif
    ...
    <article class="py-24 min-h-screen">
        <div class="container mx-auto px-6 max-w-4xl">
            ...
            <header class="text-center mb-12" data-aos="fade-down">
                ...
                <h1 class="text-4xl md:text-5xl font-bold mb-6 leading-tight gradient-text">{{ $post->getTrans('title') }}</h1>
            ...
            @if($post->featured_image)
                <div class="rounded-2xl overflow-hidden mb-12 shadow-2xl shadow-primary-900/20 border border-white/10" data-aos="zoom-in">
                    <img src="{{ Str::startsWith($post->featured_image, 'http') ? $post->featured_image : asset('storage/' . $post->featured_image) }}" alt="{{ $post->getTrans('title') }}" class="w-full h-auto object-cover max-h-[500px]">
                </div>
            @endif
        ...
        <div class="container mx-auto px-6 max-w-4xl mt-12" data-aos="fade-up">
